...
// The Adopt page groups pets by these species, so the shelter picks from a
// list instead of typing free text ("dogs", "Doggo"... would land in "Other").
...
function Pets() {
  ...
  const handleSave = async (e) => {
    ...
    if (!form.species) {
      setError("Please choose a species so adopters can find this pet.");
      return;
    }
    ...
    try {
      ...
      const petPayload = {
        ...form,
        name: form.name.trim(),
        type: form.species, // Adopt page reads `type` (falls back to `species`)
        tags: form.tags
          .split(",")
          .map((t) => t.trim())
          .filter(Boolean),
      ...
      setNotice(
        form.status === "Adopted"
          ? `${form.name.trim()} was saved. Adopted pets are hidden from the Adopt page.`
          : `${form.name.trim()} was saved and is now visible on the Adopt page.`
      );
    } catch (err) {
