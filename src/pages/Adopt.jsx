...
const EmbeddedStyles = () => (
  <style>{`
    ...
    .pc-card:hover { transform: translateY(-4px); box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); }
    .pc-img-wrap { position: relative; height: 220px; background: #f1f5f9; cursor: pointer; overflow: hidden; }
    .pc-img-wrap img { width: 100%; height: 100%; object-fit: cover; }
    .pc-badge { position: absolute; top: 12px; right: 12px; padding: 4px 10px; border-radius: 20px; font-size: 0.75rem; font-weight: 700; }
    .pc-zoom-hint { position: absolute; bottom: 8px; right: 8px; background: rgba(0, 0, 0, 0.6); color: white; font-size: 0.7rem; padding: 4px 8px; border-radius: 6px; backdrop-filter: blur(2px); }
    .pc-body { padding: 16px; display: flex; flex-direction: column; flex: 1; }
    .pc-meta { display: flex; flex-wrap: wrap; gap: 8px; font-size: 0.8rem; color: #64748b; margin-bottom: 8px; }
    .pc-name { margin: 0 0 6px 0; font-size: 1.25rem; color: #0f172a; }
    .pc-poster { font-size: 0.8rem; font-weight: 600; color: #15803d; margin-bottom: 8px; }
    .pc-desc { font-size: 0.875rem; color: #475569; margin: 0 0 16px 0; line-height: 1.4; flex: 1; }
    .pc-tags { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 16px; }
    .pc-tag-chip { background: #f1f5f9; color: #475569; font-size: 0.75rem; padding: 3px 8px; border-radius: 6px; }
    .pc-adopt-btn { width: 100%; padding: 12px; background: #15803d; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: background 0.2s; }
    .pc-adopt-btn:hover { background: #166534; }
    .pc-adopt-btn:disabled { background: #cbd5e1; cursor: not-allowed; }
    .ad-modal-overlay, .ad-zoom-overlay { position: fixed; inset: 0; background: rgba(15, 23, 42, 0.65); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; z-index: 9999; padding: 16px; }
    .ad-modal { background: white; border-radius: 16px; max-width: 520px; width: 100%; max-height: 90vh; overflow-y: auto; padding: 28px; position: relative; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); }
    ...
    .ad-modal-pet-strip { display: flex; align-items: center; gap: 16px; background: #f8fafc; padding: 12px 16px; border-radius: 10px; margin-bottom: 20px; border: 1px solid #e2e8f0; }
    .ad-modal-pet-img { width: 52px; height: 52px; border-radius: 8px; object-fit: cover; }
    .ad-modal-pet-name { font-weight: bold; color: #0f172a; }
    .ad-modal-pet-type { font-size: 0.8rem; color: #64748b; }
    .ad-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
    .ad-form-full { grid-column: span 2; }
    .ad-label { display: block; font-size: 0.8rem; font-weight: 600; margin-bottom: 6px; color: #334155; }
    .ad-input { width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; margin-bottom: 12px; font-size: 0.9rem; outline: none; }
    .ad-input:focus { border-color: #16a34a; }
    .ad-submit-btn { width: 100%; padding: 14px; background: #15803d; color: white; border: none; border-radius: 10px; font-weight: 600; font-size: 1rem; cursor: pointer; margin-top: 16px; }
    .ad-submit-btn:hover { background: #166534; }
    ...
    .ad-zoom-img { max-width: 90vw; max-height: 85vh; border-radius: 12px; object-fit: contain; }
    ...
    .ad-success { text-align: center; padding: 20px 0; }
    .ad-success-icon { font-size: 3rem; display: block; margin-bottom: 12px; }
  `}</style>
);

const getPetImgUrl = (pet) => pet?.photoUrl || pet?.image || pet?.photo || pet?.imageUrl || "";
...
function PetCard({ pet, onAdopt, onZoom }) {
  ...
  const badgeText = pet.badge || pet.status || "Available";
  ...
  return (
    <article ref={ref} className="pc-card">
      <div className="pc-img-wrap" onClick={() => onZoom(displayImage)}>
        ...
        />
        {badgeText && (
          <span className="pc-badge" style={{ background: BADGE_COLORS[badgeText]?.bg || "#e8f5e9", color: BADGE_COLORS[badgeText]?.text || "#2e7d32" }}>
            {badgeText}
          </span>
        )}
      ...
      <div className="pc-body">
        ...
        <div className="pc-poster">
          🏡 Posted by: {pet.postedByRole === "shelter" ? "Verified Shelter" : "Pet Owner / User"}
        </div>
...
export default function Adopt() {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const fetchedPets = snapshot.docs.map((doc) => {
        ...
        let cleanType = rawType
          ...
          : "Other";
        // Older listings may say "Dogs" / "Cats" — treat them as Dog / Cat.
        if (/^(dog|cat|rabbit|bird|parrot)s$/i.test(cleanType)) {
      ...
      setPets(fetchedPets.filter((p) => p.status !== "Adopted"));
  ...
  useEffect(() => {
    return onAuthStateChanged(auth, (u) => setIsLoggedIn(!!u));
  }, []);

  const getAgeInMonths = (ageStr) => {
    if (!ageStr) return 0;
    const str = String(ageStr).toLowerCase();
    const value = parseInt(str, 10) || 0;
    if (str.includes("year") || str.includes("yr")) return value * 12;
    return value;
  };
  ...
  // 1. Guard check for active user authentication
  const currentUser = auth.currentUser;
  if (!currentUser) {
    setAuthError("You must be logged in to submit an application.");
    return;
  }
  ...
  try {
    ...
    // 2. Prepare Application Document Ref
    const appRef = doc(collection(db, "applications"));
    batch.set(appRef, {
      petId: selectedPet.id,
      petName: selectedPet.name,
      petType: selectedPet.type,
      postedBy: selectedPet.postedBy || null,
      postedByRole: selectedPet.postedByRole || null,
    ...
    // 3. Prepare User Notification Ref
    ...
    // 4. Prepare Pet Owner Notification Ref (if applicable)
    const ownerId = selectedPet.postedBy || selectedPet.shelterId;
    if (ownerId && ownerId !== currentUser.uid) {
      const ownerNotifRef = doc(collection(db, "notifications"));
      batch.set(ownerNotifRef, {
        userId: ownerId,
        text: `📢 ${contactData.fullName} submitted an adoption application for your pet listing: ${selectedPet.name}.`,
        createdAt: serverTimestamp(),
    ...
    <div className="adopt-root">
      ...
      <div className="ad-layout">
        ...
        <main className="ad-main">
          ...
          {petsLoading ? (
            <div className="ad-empty"><p>Loading available pets...</p></div>
          ) : sortedPets.length === 0 ? (
            ...
            {modalStep === "adopt" && (
              <div className="ad-modal-pet-strip">
                ...
                <div>
                  ...
                  <div className="ad-modal-pet-type">
