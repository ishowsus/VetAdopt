...
function Login() {
  ...
  // Firebase Logic - No design changes here
 const handleLogin = async () => {
    ...
    try {
      ...
      // 2. Create the user object to store locally
      // Note: In a real app, you might fetch additional data from Firestore here
      const userData = {
        uid: user.uid,
        email: user.email,
      ...
      // 3. Save to localStorage so App.js sees the user
      localStorage.setItem("user", JSON.stringify(userData));

      // 4. Trigger the event to update the Navbar immediately
      window.dispatchEvent(new Event("authChange")); 
      
      // 5. Now navigate - ProtectedRoute will now allow this!
      navigate("/profile");
    } catch (error) {
      console.error(error);
